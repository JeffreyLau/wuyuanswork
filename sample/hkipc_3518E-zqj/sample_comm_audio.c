...
static HI_S32 SAMPLE_Acodec_CfgAudio(AUDIO_SAMPLE_RATE_E enSample, HI_BOOL bMicIn)
{
    ...
    unsigned int i2s_sel = 0x0; //0:disable, 1:enable.
    unsigned int clk_sel = 0x1;
    unsigned int track_select = 0x1;
    unsigned int pd_ctrl = 0x01;
    ACODEC_VOL_CTRL vol_ctrl;
    vol_ctrl.vol_ctrl_mute = 0x0; //0:disable mute, 1:mute.
...
#if 1 //this must be set for double audio trasfer.
    /**sample input & output mute control**/
    unsigned int mute_ctrl = 0x00;
    if (ioctl(fdAcodec, ACODEC_SET_MICL_MUTE, &mute_ctrl))
    {
        printf("[%s, %d] set ACODEC_SET_MICL_MUTE failed!\n", __func__, __LINE__);
        close(fdAcodec);
        return HI_FAILURE;
    }
    //if (ioctl(fdAcodec, ACODEC_SET_MICR_MUTE, &mute_ctrl))
    //{
    //    printf("[%s, %d] set ACODEC_SET_MICR_MUTE failed!\n", __func__, __LINE__);
    //    close(fdAcodec);
    //    return HI_FAILURE;
    //}
    ...
    {
        printf("[%s, %d] set ACODEC_SET_DACL_MUTE failed!\n", __func__, __LINE__);
        close(fdAcodec);
        return HI_FAILURE;
    }
    //if (ioctl(fdAcodec, ACODEC_SET_DACR_MUTE, &mute_ctrl))
    //{
    //    printf("[%s, %d] set ACODEC_SET_DACR_MUTE failed!\n", __func__, __LINE__);
    //    close(fdAcodec);
    //    return HI_FAILURE;
    //}
#endif
    ...
    {
        printf("ioctl ACODEC_SET_ADC_HP_FILTER error!\n");
    }

    if (ioctl(fdAcodec, ACODEC_SET_DAC_DE_EMPHASIS, &adc_hpf))
    {
        printf("ioctl ACODEC_SET_DAC_DE_EMPHASIS error!\n");
    }
...
#if 1
    /**volume output control left**/
    vol_ctrl.vol_ctrl = 0x16; //0x7A; /*volume control, 0x00~0x7e, 0x7F:mute*/
    if (ioctl(fdAcodec, ACODEC_SET_DACL_VOL, &vol_ctrl))
    {
        printf("[%s, %d] ioctl ACODEC_SET_DACL_VOL err!\n", __func__, __LINE__);
        close(fdAcodec);
        return HI_FAILURE;
    }

    ///**volume output control right**/
    //if (ioctl(fdAcodec, ACODEC_SET_DACR_VOL, &vol_ctrl))
    //{
    //    printf("[%s, %d] ioctl ACODEC_SET_DACR_VOL err!\n", __func__, __LINE__);
    //    close(fdAcodec);
    //    return HI_FAILURE;
    //}

    /**volume input control left**/
    //vol_ctrl.vol_ctrl = 0x33; //0x7A; /*volume control, 0x00~0x7e, 0x7F:mute*/
    if (ioctl(fdAcodec, ACODEC_SET_ADCL_VOL, &vol_ctrl))
    {
        printf("[%s, %d] ioctl ACODEC_SET_ADCL_VOL err!\n", __func__, __LINE__);
        close(fdAcodec);
        return HI_FAILURE;
    }

    ///**volume input control right**/
    //if (ioctl(fdAcodec, ACODEC_SET_ADCR_VOL, &vol_ctrl))
    //{
    //    printf("[%s, %d] ioctl ACODEC_SET_ADCR_VOL err!\n", __func__, __LINE__);
    //    close(fdAcodec);
    //    return HI_FAILURE;
    //}
#endif
...
static HI_S32 SAMPLE_Acodec_CfgAudio2222(AUDIO_SAMPLE_RATE_E enSample, HI_BOOL bMicIn)
{
    HI_S32 fdAcodec = -1;
    unsigned int i2s_fs_sel = 0;
    unsigned int mixer_mic_ctrl = 0;
    ...
    track_select = 0x1;
    if (ioctl(fdAcodec, ACODEC_DACR_SEL_TRACK, &track_select))
    {
        printf("..................................ioctl err!\n");
    }
    if (ioctl(fdAcodec, ACODEC_ADCR_SEL_TRACK, &track_select))
    {
        printf("..............................ioctl err!\n");
    }
#if 1 //this must be set for double audio trasfer.
    /**sample input & output mute control**/
    unsigned int mute_ctrl = 0x01;
    if (ioctl(fdAcodec, ACODEC_SET_MICR_MUTE, &mute_ctrl))
    {
        printf("[%s, %d] set ACODEC_SET_MICL_MUTE failed!\n", __func__, __LINE__);
        close(fdAcodec);
...
#if 1
    /**volume output control left**/
    vol_ctrl.vol_ctrl = 0x16; /*volume control, 0x00~0x7e, 0x7F:mute*/
    if (ioctl(fdAcodec, ACODEC_SET_DACL_VOL, &vol_ctrl))
    {
        printf("[%s, %d] ioctl ACODEC_SET_DACL_VOL err!\n", __func__, __LINE__);
        close(fdAcodec);
        ...
        AIO_ATTR_S *pstAioAttr, HI_BOOL bAnrEn, AUDIO_RESAMPLE_ATTR_S *pstAiReSmpAttr)
{
    ...
    #if 0 //zqj.
        s32Ret = HI_MPI_AI_EnableAec(AiDevId, i, 0, 0);
        if(HI_SUCCESS != s32Ret)
        {
            printf("%s: HI_MPI_AI_EnableAec(%d,%d) failed with %#x\n", __FUNCTION__, AiDevId, i, s32Ret);
            return HI_FAILURE;
        }
    #endif
...
HI_S32 SAMPLE_COMM_AUDIO_StartAdec(ADEC_CHN AdChn, PAYLOAD_TYPE_E enType)
{
    ...
    printf("[%s, %d] stAdecAttr.enMode: %d\n", __func__, __LINE__, stAdecAttr.enMode);
