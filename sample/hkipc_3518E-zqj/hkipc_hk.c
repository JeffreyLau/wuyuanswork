...
#if (DEV_ROBOT)
    int g_AUDIO_SET_WIFI_TIME = 60;
#else
    int g_AUDIO_SET_WIFI_TIME = 30;
#endif
...
extern int g_KeySplit;


extern int g_GetAllTfFile;
extern short g_TF_start;
...
short g_onePtz = 0;
int  g_DevVer = 523; //nvr update
short g_DevIndex = 0;
//int g_startCheckAlarm = 0;
volatile int g_AudioSetWifi = 0;
int g_irOpen = 0;
int g_NetWorkMode = 0; //0:wire;  1:wifi_sta;  2:wifi_AP;
...
#define PRESENT_STR HK_KEY_MAINCMD"=LocalData;" \
    ...
    if (!quit_)
    {
        ...
        if (1 == g_isWifiInit) //ra0.
        {
            ...
            char arySSID[1024]={0};
            strcpy( arySSID,mywifiCfg.ssid );
            untroubled( arySSID,arySSID,arySSID+strlen(arySSID) );
            SetParamStr( framePacket, HK_WIFI_SID,arySSID );
        ...
        n = sprintf(presenBuf, PRESENT_STR, eth0Addr.ipMode, eth0Addr.dns1, eth0Addr.dns2, eth0Addr.netmask, eth0Addr.gateway, x, g_DevVer,g_DevIndex, g_lanPort,g_isWanEnable,g_nSignLogin,\
                ...
                hkSdParam.haveUse,hkSdParam.leftSize,hkSdParam.audio,hkSdParam.sdrecqc,getenv("USER"),host_ ,getEnv("LAN_PASSWD","123456"),aryBuf);
        LanPresent( presenBuf, n);
    }
}

static int UpdateSystemTimestamp(void)
{
...
HI_S32 Video_SubSystem_Init(HI_VOID)
{
    ...
    ******************************************/
    ...
    stVpssChnMode.u32Height     = 360;//480;
    ...
    stVpssExtChnAttr.u32Height       = 360;//480;
...
static unsigned long gReLogIn = 0;
static void* thd_nonblock_login(void* a)
{
    ...
    while (!quit_)
    {
        ...
        if (2 == g_NetWorkMode) //wifi AP mode.
        {
            
            if(g_isWifiInit==1 && strlen(mywifiCfg.ssid) > 0 )//sun
            {
                gReLogIn++;
                if ( gReLogIn >= 20 )
                {
                    printf("[%s, %d]AP- server login failed, system reboot !!!\n", __func__, __LINE__);
                    wrap_sys_restart( );
                }
            }
            else
        ...
        else if (signcode_ != HK_LOGIN_SUCCESS)
        {
            char cBuf[64] = {0};
            HKFrameHead *framePacket = CreateFrameB();
            //printf( "login in\n" );
            ...
            gReLogIn ++;
            if ( gReLogIn > 4 ) //modyfied:2015-03-06.
            { //restart system while server login failed after 12 minutes.
                printf("[%s, %d] server login failed, system reboot !!!\n", __func__, __LINE__);
                wrap_sys_restart( );
            }
        ...
        for (x = 0; ((x < 60*3) && (!quit_)); ++x) //login server every 3 minutes.
        {
            sleep(2);
        }
    }
...
static int g_jsw_set_time=0;
static void jsw_set_time_hurt()
{
    if(g_jsw_set_time == 1 )
        return;
    g_jsw_set_time=1;
    ...
    printf("JSW set time hour=%d.min=%d....\n",iHour, iMin);
    UART_CtrlCmd_Send(CMD_ANDSON_HOUR, iHour) ;
    usleep(1000*1000);
    UART_CtrlCmd_Send(CMD_ANDSON_MINUTES, iMin) ;
...
int AudioSetWifi(char *cSid, char *cPaswd)
{
    ...
    char oldSsid[64]={0};
    char oldPwd[64]={0};
    conf_get_space( HOME_DIR"/wifisid.conf", HK_WIFI_SID, oldSsid, 64);
    conf_get_space( HOME_DIR"/wifisid.conf", HK_WIFI_PASSWORD, oldPwd, 64 );
    char cDecPswd[64] = {0};
    if( cPaswd == NULL )
    {
        strcpy( cDecPswd, "12345678" );
        if( strcmp(cSid, oldSsid)==0 )
        {
            printf("...%s.NO Passwd..wifi ssid  is already configured before !!!\n", __func__);
            return -1;
        }
    }
    else
    {
        strcpy( cDecPswd, cPaswd );
        if( strcmp(cSid, oldSsid)==0 && strcmp(cPaswd, oldPwd)==0)
        {
            printf("...%s...wifi ssid & pwd is already configured before !!!\n", __func__);
            return -1;
        }
    }
    ...
    char cSsid[64]  = {0};
    char cSafeType[32]  ={"auto"};// DictGetStr( hf, HK_WIFI_SAFETYPE );
    char *cSafeOption =NULL; // DictGetStr( hf, HK_WIFI_SAFEOPTION );
    char *cKeyType    =NULL;// DictGetStr( hf, HK_WIFI_KEYTYPE );
    //char *cPasswd     = DictGetStr( hf, HK_WIFI_PASSWORD );
    char cBootProto[32]  ={"dhcp"};// DictGetStr( hf, HK_NET_BOOTPROTO );
    char cEnctyType[32]  ={"auto"}; //DictGetStr( hf, HK_WIFI_ENCRYTYPE );
    ...
    conf_set_int(HOME_DIR"/wifinet.cfg", HK_WIFI_OPENORCLOSE, 1);

    /**check AP mode for phone set wifi connection**/
    char s_LocalIPAddr[16] = {0};
    if ( (0 == strcmp(cSafeType, "auto")) || (0 == strcmp(cSafeType, "none")) )
    {
    ...
    char ssid[128]={0};
    snprintf(ssid, sizeof(ssid), "\"%s\"", cSsid);

    char psk[128]={0};
    snprintf(psk, sizeof(psk), "\"%s\"", cDecPswd);

    if (0 == strcmp(cSafeType, "none")) //authentication:NONE
    {

        system("cp -f /etc/wifiConf/open.conf /mnt/sif/");
        usleep(1000);
    ...
    {
        system("cp -f /etc/wifiConf/wep.conf /mnt/sif/");
        usleep(1000);
        conf_set_space(HOME_DIR"/wep.conf", "ssid", ssid);
        conf_set_space(HOME_DIR"/wep.conf", "wep_key0", psk);
        conf_set_space(HOME_DIR"/wep.conf", "wep_key1", psk);
        conf_set_space(HOME_DIR"/wep.conf", "wep_key2", psk);
        conf_set_space("/etc/Wireless/RT2870STA/RT2870STA.dat", "SSID", cSsid);
        conf_set_space("/etc/Wireless/RT2870STA/RT2870STA.dat", "WPAPSK", cDecPswd);
        u32Ret = 1;
    }
    else //authentication:WPA-PSK or WPA2-PSK
    {
        if (strlen(psk) < 10)
        {
            ...
            conf_set_space(HOME_DIR"/wpa_supplicant.conf", "psk", "\"1234567890\"");
            u32Ret = 0;
        }
        else
        {
            system("cp -f /etc/wifiConf/wpa_supplicant.conf /mnt/sif/");
            usleep(1000);
            conf_set_space(HOME_DIR"/wpa_supplicant.conf", "psk", psk);
            conf_set_space(HOME_DIR"/wpa_supplicant.conf", "ssid", ssid);
            conf_set_space("/etc/Wireless/RT2870STA/RT2870STA.dat", "SSID", cSsid);
            conf_set_space("/etc/Wireless/RT2870STA/RT2870STA.dat", "WPAPSK", cDecPswd);
            u32Ret = 1;
        }
...
static int cb_lan(unsigned int hkid, int asc, const char* buf, unsigned int len)
{
    ...
    else if ( strcmp(mc, "700") == 0 ) //set wifi.
    {
        printf("=============== 700 =================\n");
        const char* mac = GetParamStr(hf, HK_KEY_HWADDR);
        if (mac)
        {
            ...
            if (isMacMatch(mac))
            {
                ...
                if (1 == isOpenWifi)
                {
                    ...
                    //if ((!(strcmp(cSafeType, "auto"))) || (!(strcmp(cEnctyType, "none")) && (!getLocalAddress("ra0", s_LocalIPAddr)) && (strlen(s_LocalIPAddr) > 0) && (0 == strcmp(s_LocalIPAddr, IPADDR_APMODE))))
                    if ( (0 == strcmp(cSafeType, "auto")) || (0 == strcmp(cSafeType, "none")) )
                    {
                        HK_DEBUG_PRT("+++++++++++++++++ phone setting wifi connection +++++++++++++++++\n");
                        int wificount = 0; //index the number of wifi node.
                    ...
                    if (0 == strcmp(cSafeType, "none")) //authentication:NONE
                    {
                        ...
                        system("cp -f /etc/wifiConf/open.conf /mnt/sif/");
                        usleep(10000);
                        conf_set_space(HOME_DIR"/open.conf", "ssid", ssid);
                    }
                    else if (0 == strcmp(cSafeType, "wep")) //authentication:WEP
                    {
    ...
    else if ( strcmp(mc, "703")==0 ) //wifi scan.
    {
            ...
            if (isMacMatch( mac ))
            {
                ...
                if ((!getLocalAddress("ra0", s_LocalIPAddr)) && (strlen(s_LocalIPAddr) > 0) && (0 == strcmp(s_LocalIPAddr, IPADDR_APMODE)))
                {
                    ...
                    for (i = 0; i < iCount; i++)
                    {
                        ...
                        char arySSID[1024]={0};
                        strcpy( arySSID,cSsid );
                        untroubled( arySSID,arySSID,arySSID+strlen(arySSID) );

                        SetParamStr( hfwi, aryKey, arySSID );
                    ...
                    char *cBuf2 = malloc( iPacketLen + 100 );
                    if (NULL != cBuf)
                    {
                        GetFramePacketBuf( hfwi, cBuf, iPacketLen );
                        strEncode( cBuf,cBuf2 );
                        be_present2( iPacketLen, cBuf2, "LocalWifi", ulParam );
                        free( cBuf );
                        free( cBuf2 );
                        cBuf = NULL;
                    }
...
void GetAlarmEmailInfo()
{
    ...
    int isOpen  =1;// conf_get_int(HOME_DIR"/emalarm.conf", HK_KEY_ISALERT_SENSE);
...
static void initGPIO()
{
    ...
    nRet = Hi_SetGpio_GetBit( groupnum, bitnum, &val_read );//ircut1_grp:0-2.
	HK_DEBUG_PRT("...Get GPIO %d_%d  read Value: %d...\n", groupnum, bitnum, val_read);

    groupnum = 0;
    bitnum = 1; //GPIO:0_1 (ircut2).
	...
	/**init IO alarm**/
	groupnum = 5;
	bitnum   = 2; //GPIO:5_2 ==> IO alarm In.
	nRet = Hi_SetGpio_SetDir( groupnum, bitnum, GPIO_READ );
	nRet = Hi_SetGpio_GetBit( groupnum, bitnum, &val_read );
	HK_DEBUG_PRT("...Get GPIO %d_%d  read Value: %d...\n", groupnum, bitnum, val_read);

	groupnum = 5;
	bitnum   = 3;  //GPIO:5_3 ==> IO alarm Out.
	val_set  = 0;  //pull down.
	Hi_SetGpio_SetDir( groupnum, bitnum, GPIO_WRITE );
	Hi_SetGpio_SetBit( groupnum, bitnum, val_set );
	HK_DEBUG_PRT("....Set GPIO %d_%d  set Value: %d....\n", groupnum, bitnum, val_set);
...
#if (DEV_YANXIN)
    groupnum = 0;
    bitnum   = 5; //GPIO:0_5 (system start).
    val_set  = 1; //pull up.
    Hi_SetGpio_SetDir( groupnum, bitnum, GPIO_WRITE );
    Hi_SetGpio_SetBit( groupnum, bitnum, val_set );
    HK_DEBUG_PRT("....Set GPIO %d_%d  set Value: %d....\n", groupnum, bitnum, val_set);
...
#if (HK_PLATFORM_HI3518E | DEV_ANDSON)
    /**init RUN light**/
    groupnum = 5;
    bitnum   = 3; //GPIO:5_3.
    val_set  = 0; //pull down.
...
//#if (DEV_DOORBELL) //zhong xiang doorbell.
#if (DEV_DOORBELL | DEV_BAFU) //zhong xiang doorbell.
    groupnum = 7;
	bitnum   = 7;  //GPIO:7_7 ==> 433
	val_set  = 1;  //up.
	Hi_SetGpio_SetDir( groupnum, bitnum, GPIO_WRITE );
	Hi_SetGpio_SetBit( groupnum, bitnum, val_set );
	HK_DEBUG_PRT("....Set GPIO %d_%d  set Value: %d....\n", groupnum, bitnum, val_set);
    ...
    nRet = Hi_SetGpio_SetDir( groupnum, bitnum, GPIO_READ );
    nRet = Hi_SetGpio_GetBit( groupnum, bitnum, &val_read );
	HK_DEBUG_PRT("....Get GPIO %d_%d  read SOS Value: %d....\n", groupnum, bitnum, val_read);
    ...
    nRet = Hi_SetGpio_SetDir( groupnum, bitnum, GPIO_READ );
    nRet = Hi_SetGpio_GetBit( groupnum, bitnum, &val_read );
	HK_DEBUG_PRT("....Get GPIO %d_%d  read PIR Value: %d....\n", groupnum, bitnum, val_read);
#endif
...
static int hk_IrcutCtrl(int nboardtype)
{
    ...
    int val_read = -1, curState = 0;
    unsigned int val_set = 0;
    unsigned int groupnum = 0;
    unsigned int bitnum = 0;
    short nRet = 0;
#if (DEV_KELIV) //klv 38*38.
    unsigned int gpio_grp_ircutIN = 0;
    unsigned int gpio_bit_ircutIN = 0; //IRCUT IN GPIO:0_0.
    unsigned int gpio_grp_ircut0 = 0;
    unsigned int gpio_bit_ircut0 = 1; //GPIO:0_1 (ircut-)
    unsigned int gpio_grp_ircut1 = 0;
    unsigned int gpio_bit_ircut1 = 2; //GPIO:0_2 (ircut+)
#else //hk 3518e.
    unsigned int gpio_grp_ircutIN = 1;
    unsigned int gpio_bit_ircutIN = 7; //IRCUT IN GPIO:1_7.
    #if (DEV_DOORBELL)
    unsigned int gpio_grp_ircut0 = 2;
    unsigned int gpio_bit_ircut0 = 2; //GPIO:2_2 (ircut-)
    unsigned int gpio_grp_ircut1 = 2;
    unsigned int gpio_bit_ircut1 = 4; //GPIO:2_4 (ircut+)
    #else
    unsigned int gpio_grp_ircut0 = 2;
    unsigned int gpio_bit_ircut0 = 4; //GPIO:2_4 (ircut-)
    unsigned int gpio_grp_ircut1 = 2;
    unsigned int gpio_bit_ircut1 = 2; //GPIO:2_2 (ircut+)
    #endif
#endif

    if (!g_bIrcutInit) //ali
    {
        nRet = Hi_SetGpio_SetDir( gpio_grp_ircutIN, gpio_bit_ircutIN, GPIO_READ );
        nRet = Hi_SetGpio_SetDir( gpio_grp_ircut0, gpio_bit_ircut0, GPIO_WRITE );
        nRet = Hi_SetGpio_SetDir( gpio_grp_ircut1, gpio_bit_ircut1, GPIO_WRITE );
    }

    /**read ircut status**/
    nRet = Hi_SetGpio_GetBit( gpio_grp_ircutIN, gpio_bit_ircutIN, &val_read );
	//HK_DEBUG_PRT("...nboardtype:%d, Get GPIO %d_%d  read Value: %d...\n", nboardtype, gpio_grp_ircutIN, gpio_bit_ircutIN, val_read);

    if (1 == g_irOpen) //disable IRCut ctrol, stay in day mode.
    {
        val_set  = 1; //pull up.
        Hi_SetGpio_SetBit( gpio_grp_ircut0, gpio_bit_ircut0, val_set );
        //HK_DEBUG_PRT("....Set GPIO %d_%d  set Value: %d....\n", gpio_grp_ircut0, gpio_bit_ircut0, val_set);
        usleep(1000*200);
        val_set = 0; //pull down.
        Hi_SetGpio_SetBit( gpio_grp_ircut0, gpio_bit_ircut0, val_set );
        //HK_DEBUG_PRT("....Set GPIO %d_%d  set Value: %d....\n\n", gpio_grp_ircut0, gpio_bit_ircut0, val_set);
        ...
        if (0 == nboardtype) //ircut light board type: level 0.
        {
            ...
            if (0 == val_read) //day mode.
            {
                ...
                if (4 == g_IRCutStateDayCnt++) 
                {
                    ...
                    HISI_SetTurnColor(0, 2);//wangshaoshu
                    //wss 20151014
                    bzSharpness = conf_get_int("/mnt/sif/hkipc.conf","SharpnessLevel");//38			
		      HISI_SetSharpNess(bzSharpness); 
                    //#if ((DEV_INFRARED) || (DEV_BAFU))
                    #if (DEV_INFRARED)
                        VISP_SetDemosaic(1000);
                    #endif
                }
            }
            ...
            {
                g_IRCutStateDayCnt = 0;
                //if (10 == g_IRCutStateNightCnt++)
                if (8 == g_IRCutStateNightCnt++)
                {
                    g_IRCutStateNightCnt = 0;
                    ...
                    //wss 20151014
                    HISI_SetSharpNess(28);	
                    //#if ((DEV_INFRARED) || (DEV_BAFU))
		      #if (DEV_INFRARED)
                        VISP_SetDemosaic(3000);
                    #endif
                    sleep(1);

                    val_set  = 1; //pull up.
                    Hi_SetGpio_SetBit( gpio_grp_ircut1, gpio_bit_ircut1, val_set );
                    //HK_DEBUG_PRT("....Set GPIO %d_%d  set Value: %d....\n", gpio_grp_ircut1, gpio_bit_ircut1, val_set);
                    usleep(1000*200);
                    val_set = 0; //pull down.
                    Hi_SetGpio_SetBit( gpio_grp_ircut1, gpio_bit_ircut1, val_set );
                    HK_DEBUG_PRT("....Set GPIO %d_%d  set Value: %d....\n\n", gpio_grp_ircut1, gpio_bit_ircut1, val_set);
...
static void CheckIOAlarm()
{
    ...
    if (0 == val_read) //IO is Alarming.
    {
        ...
        //CheckAlarm(0,4,0,NULL); //4:SOS KEY Alarm
        CheckAlarm(0,6,0,NULL);   //6:m433 alarm
...
/***********************************************
 * func: check key(GPIO0_6) press for system reboot.
 **********************************************/
int HK_Check_Key_PowerDown(void)
{
    unsigned int val_read = 0;
    unsigned int groupnum = 0;
    unsigned int bitnum   = 6; //GPIO0_6.
    short nRet = 0;

    nRet = Hi_SetGpio_SetDir( groupnum, bitnum, GPIO_READ );
    nRet = Hi_SetGpio_GetBit( groupnum, bitnum, &val_read );
    HK_DEBUG_PRT("scc...................Get GPIO %d_%d  read Value: %d...\n", groupnum, bitnum, val_read);
    if (0 == val_read)
    {
        unsigned int groupnum1 = 0;
        unsigned int bitnum1   = 5;
        unsigned int val_set   = 0;
        Hi_SetGpio_SetDir( groupnum1, bitnum1, GPIO_WRITE );
        Hi_SetGpio_SetBit( groupnum1, bitnum1, val_set ); //pull down.
        //HK_DEBUG_PRT("....Set GPIO %d_%d  set Value: %d....\n\n", groupnum1, bitnum1, val_set);
    }
...
#if (DEV_DOORBELL)
/***********************************************
 * func: check key pressed state for alarming.
 **********************************************/
int HK_Check_SOSKey()
{
    unsigned int groupnum = 0;
    ...
    bitnum = 6; //GPIO:7_6.
    nRet = Hi_SetGpio_SetDir( groupnum, bitnum, GPIO_READ );
    nRet = Hi_SetGpio_GetBit( groupnum, bitnum, &val_read ); //SOS key: GPIO0_6.
    if (1 == val_read) //SOS key pressed.
    {
        printf("Read 7_6...%s...SOS Key Alarming.nRet=%d.val_read=%d....\n", __func__,nRet, val_read);
    ...
    if (nRet < 0)
    {
        HK_DEBUG_PRT("Get GPIO %d_%d error!\n", groupnum, bitnum);
        return -1; 
    }
    if (0 == val_read) //SOS key pressed.
    {
        printf("0_6......%s...SOS Key Alarming......\n", __func__);
        CheckAlarm(0, 4, 0, NULL);
    }
...
/*****************************************************
 * func: check PIR status, alarming while PIR level 
 *       changed and motion detected.
 * return: 
 *       0: not detect PIR alarm.
 *       1: PIR alarming.
 ****************************************************/
int HK_Check_PIR_State()
{
    unsigned int groupnum = 0;
    unsigned int bitnum = 0;
    unsigned int val_read = 1;
    ...
    bitnum = 4; //GPIO:1_4.
    nRet = Hi_SetGpio_SetDir( groupnum, bitnum, GPIO_READ );
    nRet = Hi_SetGpio_GetBit( groupnum, bitnum, &val_read ); //PIR: GPIO1_4.
    if (nRet < 0)
    {
        HK_DEBUG_PRT("Get GPIO %d_%d error!\n", groupnum, bitnum);
        return -1; 
    ...
    if (1 == val_read) //PIR level changed.
    {
        //printf("......%s...PIR...Alarming......\n", __func__);
        CheckAlarm(0, 4, 0, NULL);
        return 1; //PIR Alarming.
    }
    else
    {
        return 0; //not detect PIR.
    }

    return 0;
}

int HK_Check_PIR_State_8029()
{
    unsigned int groupnum = 0;
    unsigned int bitnum = 0;
    unsigned int val_read = 1;
    ...
    bitnum = 5; //GPIO:7_5.
    nRet = Hi_SetGpio_SetDir( groupnum, bitnum, GPIO_READ );
    nRet = Hi_SetGpio_GetBit( groupnum, bitnum, &val_read ); //PIR: GPIO7_5.
    if (nRet < 0)
    {
        HK_DEBUG_PRT("Get GPIO %d_%d error!\n", groupnum, bitnum);
        return -1; 
...
#if (DEV_BAFU)
/******************************************************
 * func: check key(GPIO1_4) state to split 
 *       and create new file to record.
 ******************************************************/
int HK_Check_Record_Split_Key()
{
    unsigned int groupnum = 1;
    unsigned int bitnum   = 4; //GPIO:1_4.
    unsigned int val_read = 0;
    short nRet = 0;

    nRet = Hi_SetGpio_SetDir( groupnum, bitnum, GPIO_READ );
    nRet = Hi_SetGpio_GetBit( groupnum, bitnum, &val_read ); //stop SD record key: GPIO1_4.
    if (nRet < 0)
    {
        HK_DEBUG_PRT("Get GPIO %d_%d error!\n", groupnum, bitnum);
        return -1; 
    }
	//HK_DEBUG_PRT("...Get GPIO %d_%d  read Value: %d, g_KeySplit: %d...\n", groupnum, bitnum, val_read, g_KeySplit);

    if (0 == val_read) //key(GPIO1_4) pressed.
    {
        //printf("...%s...key down, stop SD record......\n", __func__);
        g_KeySplit = 1;
        printf("...%s...key down, SD record split, g_KeySplit: %d......\n", __func__, g_KeySplit);
        sync();
    }
...
static void hk_set_system_time()
{
    int tz = conf_get_int(HOME_DIR"/time.conf", "zone");
...
static void init_conf()
{
    ...
    system("touch /mnt/sif/m433.conf");
    system("touch /mnt/sif/m433Code.conf");
}
...
static void hk_load_sd()
{
    ...
    if (1 == g_sdIsOnline)
    {
        ...
        HK_Check_SD_Local_Update(); //update system by sd local files.
    }
...
static int sccUpdateServerAdd()
{
    ...
    if( user_ != NULL && host_ !=NULL )
    {
        ...
        if( strcmp(host_, "bluestork.scc21.net") == 0 )
        {
            char cUpdateUrl[256]={0};
            conf_get( HOME_DIR"/hkclient.conf", "UpdateUrl", cUpdateUrl, 256 );
            if( strcmp( cUpdateUrl, "http://bluestork.scc21.net:8800/app/experience/download.jsp?fileName=h3518e_bs.txt" ) !=0 )
            {
                conf_set(HOME_DIR"/hkclient.conf", "UpdateUrl", "http://bluestork.scc21.net:8800/app/experience/download.jsp?fileName=h3518e_bs.txt" );
            }
            return;
        }
        if( strcmp(host_, "www.quintezz.info") == 0 )
        {
            char cUpdateUrl[256]={0};
            conf_get( HOME_DIR"/hkclient.conf", "UpdateUrl", cUpdateUrl, 256 );
            if( strcmp( cUpdateUrl, "http://www.quintezz.info:8800/app/experience/download.jsp?fileName=h3518e_522.txt" ) !=0 )
            {
                conf_set(HOME_DIR"/hkclient.conf", "UpdateUrl", "http://www.quintezz.info:8800/app/experience/download.jsp?fileName=h3518e_522.txt" );
            }
            return;
        }

        if( strcmp(host_, "greentech.scc21.net") == 0 )
        {
            char cUpdateUrl[256]={0};
            conf_get( HOME_DIR"/hkclient.conf", "UpdateUrl", cUpdateUrl, 256 );
            if( strcmp( cUpdateUrl, "http://greentech.scc21.net:8800/app/experience/download.jsp?fileName=h3518e_522.txt" ) !=0 )
            {
                conf_set(HOME_DIR"/hkclient.conf", "UpdateUrl", "http://greentech.scc21.net:8800/app/experience/download.jsp?fileName=h3518e_522.txt" );
            }
            return;
        }


        if( strcmp(host_,"technaxx.scc21.net") == 0 )
        {
            char cUpdateUrl[256]={0};
            conf_get( HOME_DIR"/hkclient.conf", "UpdateUrl", cUpdateUrl, 256 );
            if( strcmp( cUpdateUrl, "http://www.uipcam.com/app/experience/download.jsp?fileName=h3518e.txt" )==0 )
            {
                conf_set(HOME_DIR"/hkclient.conf", "UpdateUrl", "http://technaxx.scc21.net:8800/app/experience/download.jsp?fileName=h3518e_10.txt" );
            }
            else if( strcmp( cUpdateUrl, "http://www.uipcam.com/app/experience/download.jsp?fileName=h3518e_10.txt" )==0 )
            {
                conf_set(HOME_DIR"/hkclient.conf", "UpdateUrl", "http://technaxx.scc21.net:8800/app/experience/download.jsp?fileName=h3518e_10.txt" );
            }
            return;
        ...
        if( strcmp(host_,"camera.vije.co.kr") != 0 )
        {
            ...
            if( strcmp( cUpdateUrl, "http://www.uipcam.com/app/experience/download.jsp?fileName=h3518e.txt" )==0 )
            {
                conf_set(HOME_DIR"/hkclient.conf", "UpdateUrl", "http://www.uipcam.com/app/experience/download.jsp?fileName=h3518e_10.txt" );
            }
        }
        else if( p_temp!=NULL)
        {
...
static void setLedOrSleep()
{
    AlarmVideoRecord( false );

    if( g_NetWorkMode == 2 )//AP
    {
        if( g_AUDIO_SET_WIFI_TIME != 180 )
            g_AUDIO_SET_WIFI_TIME = 180;
    }
#if (DEV_INFRARED)
    if(g_KeyResetCount != 0 )
    {
        int groupnum = 1;
        int bitnum   = 6; //GPIO:1_5.
    ...
    {
        int groupnum = 1;
        int bitnum   = 5;  //GPIO:7_7 ==> light
        static int val_set  = 1;  //up.
        int i=0;
        if( g_NetWorkMode == 2 )//AP
        {
            if( g_AUDIO_SET_WIFI_TIME != 180 )
...
#if (HK_PLATFORM_HI3518E | DEV_ANDSON | DEV_ROBOT)
    static int val_se  = 1;  //up.
    int j=0;
    if(g_AudioSetWifi != g_AUDIO_SET_WIFI_TIME)
    {
        for(j=0; j<8; j++)//connect
        {
            val_se  = 1; 
            Hi_SetGpio_SetDir(g_RUN_grp, g_RUN_bit, GPIO_WRITE );
            Hi_SetGpio_SetBit(g_RUN_grp, g_RUN_bit, val_se );
            usleep(1000*125);   //
    ...
    if(g_NetWorkMode == 1|| g_NetWorkMode == 0)//wifi sta
    {
        val_se  = !val_se;
        Hi_SetGpio_SetDir( g_RUN_grp, g_RUN_bit, GPIO_WRITE );
        Hi_SetGpio_SetBit( g_RUN_grp, g_RUN_bit, val_se );
...
{  
    ...
    if (1 == m433enable)
    {
        ...
        sleep(2);
        if (0 == init_m433(getpid()))
        {
            printf("insmod 433 driver success !\n");
        }
        else
        {
        #if (HK_PLATFORM_HI3518E)
            ...
            unsigned int val_read = 0;
            int groupnum = 7;
            int bitnum   = 6; //GPIO:7_6 ==> IO alarm In.
            int nRet = Hi_SetGpio_SetDir( groupnum, bitnum, GPIO_READ );
            nRet = Hi_SetGpio_GetBit( groupnum, bitnum, &val_read );
            HK_DEBUG_PRT("...Get GPIO %d_%d  read Value: %d...\n", groupnum, bitnum, val_read);
        ...
        #endif
            m433enable = 0;
            printf("insmod 433 driver failed, m433enable: %d\n", m433enable);
        }
    }
    else 
    {   
    #if ( DEV_INFRARED)
        // check 433 plug in ,add by ali 20150430
        system("insmod /mnt/sif/bin/hi_m433.ko");
        if( 0 == init_m433( getpid() ) )
        {
            printf( "fxb-------------found  111 \n" );
            m433enable = 1;
        }
        uninit_m433();
        //sleep(1);
        system("rmmod /mnt/sif/bin/hi_m433.ko");
        system("rmmod hi_m433");
        system("rmmod m433");
        system("rmmod hi_m433.ko");
    #endif
    }
    printf("============> m433enable: %d <=============\n", m433enable);
#endif //end by yy
...
/*<<<<<<<<<<<<<<<<<<<<<<<<<<main loop>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
    for ( ; !quit_; counter++)
    {
        ...
        {
            g_OpenSd++;
            if (g_OpenSd > 20 && g_GetAllTfFile==0)
            {
                g_OpenSd = 0;
                b_OpenSd = false;
    ...
    #if (DEV_BAFU)
        HK_Check_Record_Split_Key();
    #endif

    #if 0
        HK_Check_PIR_State_8029();
    #endif

    #if (DEV_SHANGHAI)
        HK_Check_Key_PowerDown(); //press key to power down system. 
    #endif
        ...
        setLedOrSleep();//Led Or Sleep(2)
    
        //DDNS update
        time(&tmSTopDDNS);
        if (DdnsTimeInterval*60*60 < (tmSTopDDNS - tmStartDDNS))
        {
            tmStartDDNS = tmSTopDDNS;
            IPCAM_StartDdns();
        }
        ...
        if(g_AudioSetWifi < g_AUDIO_SET_WIFI_TIME)//audio wifi set
        {
            g_AudioSetWifi++;
        }
    }
...
static void on_resp_reqinit1(Dict* d)
{
    ...
    if (ts.tv_sec > 0)
    {
        ...
        if (ec == 0)
        {
            ...
            const char *mac1     = DictGetStr( d, HK_KEY_MAC1 );
            const char *mac2     = DictGetStr( d, HK_KEY_MAC2 );
            int iM433 = DictGetInt(d, HK_KEY_IS433);
            int iWifiOpen        = DictGetInt( d, HK_WIFI_OPENORCLOSE );
            ...
            if (usr && pwd)
            {
                ...
                {
                    conf_set(HOME_DIR"/hkclient.conf", "PROXY", pHost);
                    if( strcmp(pHost, "www.quintezz.info") == 0 )
                    {
                        conf_set(HOME_DIR"/web.conf", "IpcPlugDownLoad", "http://www.quintezz.info:8800/app/experience/download.jsp?fileName=IPC.zip");
                    }
                }
            ...
            conf_set_int(HOME_DIR"/hkipc.conf", "m433enable", iM433); //level 0.
