...
#if (DEV_ROBOT)
    if ( (nFrameRate <= 0) || (nFrameRate > 10) )
    {
        nFrameRate = 10; //jisiwei robot.
    }
#else
...
#if (DEV_BAFU)
	stDenoiseAttr.u8ThreshMax = 0x60;//96;//84;
	stDenoiseAttr.u8SnrThresh[0] = 0x23;//35;
	stDenoiseAttr.u8SnrThresh[1] = 0x28;//40;
	stDenoiseAttr.u8SnrThresh[2] = 0x2B;//43;
	stDenoiseAttr.u8SnrThresh[3] = 0x60;//96;
	stDenoiseAttr.u8SnrThresh[4] = 0x46;//70;
	stDenoiseAttr.u8SnrThresh[5] = 0x48;//72;
	stDenoiseAttr.u8SnrThresh[6] = 0x4a;//74;
	stDenoiseAttr.u8SnrThresh[7] = 0x50;//80;
#else
//#if 1 // 1
...
HI_S32 VISP_SetDemosaic(int AnalogGain)
{
	int nRet = -1;

	ISP_DEMOSAIC_ATTR_S stDemosaicAttr;
	nRet = HI_MPI_ISP_GetDemosaicAttr(&stDemosaicAttr);
	if (nRet != HI_SUCCESS)
	{
		SAMPLE_PRT("HI_MPI_ISP_GetDenoiseAttr failed with %#x!\n", nRet);
		return HI_FAILURE;
	...
	if(AnalogGain >= 2500)
	{
		stDemosaicAttr.u8UuSlope = 0x72;
	}
	else 
	{
		stDemosaicAttr.u8UuSlope = 0xc8;
	}

	nRet = HI_MPI_ISP_SetDemosaicAttr(&stDemosaicAttr);
	if (nRet != HI_SUCCESS)
	{
		SAMPLE_PRT("HI_MPI_ISP_SetDenoiseAttr failed with %#x!\n", nRet);
		return HI_FAILURE;
...
HI_S32 VISP_SetVPSSGrpParam_EX(int AnalogGain)
{
	VPSS_GRP VpssGrp = 0;
	...
	int SfStrength[3] 	= {32,40,96};//{32,40,64,80};
	int TfStrength[3] 	= {8,16,24};//{8,16,16,20};
	int ChromaRange[3] 	= {8,16,48};//{8,16,16,40};
	int nRet = -1;
...
#if (DEV_INFRARED)
	VISP_SetVPSSGrpParam_EX(stInnerStateInfoEx.u32AnalogGain);
    //VISP_SetDemosaic(stInnerStateInfoEx.u32AnalogGain);
#else
...
int SaveJPEGPicsOnSDCard(char *pJpegData, int JpegSize, int num)
{
    ...
    HK_DEBUG_PRT("######## %s...JPEG size = %d......num = %d #########\n", __func__, JpegSize, num);
...
#if 1 //(DEV_INFRARED) //zqj 2015-06-12 add for YSM.
int SaveJPEGPicsOnSDCard_EX(VENC_STREAM_S *pstStream, unsigned int PicNum)
{
    char cTime[32] = {0};
    FILE *pFile_Snap = NULL;
    char acFile[64] = {0};
	...
	time(&timep);
 	p = gmtime(&timep);
	sprintf(cTime, "%d%02d%02d%02d%02d%02d", (1900+p->tm_year), (1+p->tm_mon), p->tm_mday, p->tm_hour, p->tm_min, p->tm_sec);
    //sprintf(acFile, PATH_SNAP"/m433/snap_%s.jpg", cTime); //mnt/mmc/snapshot/m433/201506121400.jpg
    sprintf(acFile, PATH_M433_SNAPSHOT"/snap_%s.jpg", cTime); //mnt/mmc/snapshot/m433/201506121400.jpg
    printf("...%s...file: %s...\n", __func__, acFile);
    ...
    s32Ret = SAMPLE_COMM_VENC_SaveJPEG(pFile_Snap, pstStream);
    if (HI_SUCCESS != s32Ret)
    {
...
/*****************************************************************
 * func: check snapshot dirctory for saving 433 alarm pictures.
 * return:
 *      0: directory is not existed.
 *      1: dir existed.
 *****************************************************************/
static int Check_Snapshot_Directory(void)
{
    if (access(PATH_SDRECORD, F_OK|R_OK|W_OK) < 0)
    {
        printf("[%s] SD card is not inserted !!!\n", __func__);
        g_EnableSnapImg = 0;
        return 0;
    }

    if (access(PATH_M433_SNAPSHOT, F_OK|R_OK|W_OK) < 0)
    {
        printf("create new dir: %s\n", PATH_M433_SNAPSHOT);                             
        if (mkdir(PATH_M433_SNAPSHOT, 0755) < 0) //create m433 dir to save snapshots.  
        {
            printf("mkdir:%s failed, %s\n", PATH_M433_SNAPSHOT, strerror(errno));       
            return -1;                                                    
        }
        else
        {
            printf("create new dir: %s succuss !\n", PATH_M433_SNAPSHOT);                             
            return 1;
        }
    }
    else
    {
        printf("[%s] %s existed !\n", __func__, PATH_M433_SNAPSHOT);
        return 1;
    }
}
...
/************************************************************
 * func: snap warn pictures for 433 alarm. (zqj)
 ************************************************************/
int HK_Snap_Warning_Pictures(void)
{
    if (1 != Check_Snapshot_Directory())
    {
        return 0; 
    }
	...
	HI_U32 i = 0;
	unsigned int streamSize = 0;
    HI_S32 SnapFd = -1; //picture snap fd.
    unsigned int JpegPicCount = 0;
    //unsigned int nSnapCount = 0;
				
	HI_S32 s32Ret;
	VENC_CHN SnapChn = 2; //JPEG enc channel:2
	...
	pstPack = (VENC_PACK_S*)malloc(sizeof(VENC_PACK_S) * 5);
	if (NULL == pstPack)
	{
	...
	SnapFd = COMM_Get_VencStream_FD( SnapChn );
	if (SnapFd < 0)		
    {
		return HI_FAILURE;
    }

    printf("[%s, %d] g_EnableSnapImg: %d, JpegPicCount: %d...\n", __func__, __LINE__, g_EnableSnapImg, JpegPicCount);
	while (g_EnableSnapImg)
	{
		FD_ZERO(&read_fds);
		FD_SET(SnapFd, &read_fds);
		s32Ret = select(SnapFd+1, &read_fds, NULL, NULL, NULL);
		if (s32Ret < 0) 
		{
			SAMPLE_PRT("select failed!\n");
			return HI_FAILURE;
		...
		else
		{
			if (FD_ISSET(SnapFd, &read_fds))
			{
				...
				/***save JPEG pictures to sd card for FTP upload***/
				//if ((1 == g_EnableSnapImg) && (nSnapCount++ > 3))
				if (1 == g_EnableSnapImg)
				{
                    //printf("...Save JPEG Pictures on SD card, JpegPicCount: %d...\n", JpegPicCount);
                    s32Ret = SaveJPEGPicsOnSDCard_EX(&stStream, JpegPicCount);
                    if (HI_SUCCESS == s32Ret)
                    {
                        JpegPicCount ++;
                        if (JpegPicCount >= 3) //snap 3 pictures while 433 alarming.
                        {
                            g_EnableSnapImg = 0; //to stop while.
                            //nSnapCount = 0;
                            JpegPicCount = 0;
                        }
                    }
...
static int g_jswRunFlag = 0;
//send jsw 106-1;107-2;108-3
//108-5; 109-4; 107-3; 105-2; 106-1
int sccSendPowerToApp(int nReserved)
{
    if( nReserved== 127 || nReserved== 108 )
        nReserved = 108; //5

    if( nReserved != -1 )
    {
        if( nReserved == 108 || nReserved == 107 ) 
        {
            if(g_jswRunFlag == 1 )
            {
                g_jswRunTime++;
        ...
        g_jswPowerFlag = nReserved;
    }
    if( g_jswPowerFlag == 108 && g_jswRunTime >= 30 )
    {
        g_jswRunTime = 0;
        g_jswPowerFlag = 109;
    }
    if( g_jswPowerFlag == 107 && g_jswRunTime >= 30 )
    {
        g_jswRunTime = 0;
        g_jswPowerFlag = 105;
    }
    printf("[%s, %d] power=%d, count=%d \n", __func__, __LINE__, g_jswPowerFlag,g_jswRunTime);
    //
    if ((g_isH264Open==2 || g_isMjpegOpen == 2 ||nReserved== -1 )&& g_jswPowerFlag > 0) 
    {
        sccLocalAlarm(0, POWER_CMD, g_jswPowerFlag, NULL);//105
        raise_alarm_server( POWER_CMD, g_jswPowerFlag, NULL);
    }
}
...
static unsigned long g_PtzTimeOld = 0;
static unsigned long g_PtzTimeNew = 0;


static void sccSetM433Ptz(int nCode )
{
    int i=0;
    for(i=0; i< 32; i++)
    {
        ...
        char cTemCode[1024]={0};
        conf_get( HOME_DIR"/m433Code.conf", keyBuf, cTemCode, 1024);
        if(strlen(cTemCode) > 2 )
        {
            char cCode[64]={0};
            char cHome[12]={0};
            char cHomeFlag[12]={0};
            char cPoint[12]={0};
            sscanf(cTemCode, "%[^-]-%[^-]-%[^-]-%[^-]", cCode, cHome, cHomeFlag,cPoint );
            int  iCode = atoi(cCode );
            int iPoint = atoi(cPoint);

            if( nCode == iCode && iPoint > 0 )
            {
                printf("se m433 count=%d buf=%d.ipoint=%d..\n", nCode, iCode, iPoint);
                g_PtzRotateEnable = 1;
                g_PtzRotateType = 0;
                g_PtzStepType = 0;	
                g_PtzPresetPos = iPoint;
            }
        }
    }
...
{
    //printf(" alarm nAlarmType=%d. nReserved=%d...\n", iType, nReserved);
    ...
    if ((iType == 4) || (iType == 6))// 4 i/o out dev alarm; 6 433 alarm.
    {
        ...
        if ((g_isH264Open == 2) || (g_isMjpegOpen == 2))
    ...
    */
    ...
    if (cur - raise_time > 2000)
    {
        ...
        if (1 == iType)
        {
            UART_CtrlCmd_Send(CMD_ANDSON_MD, 0); //motion detect.
        }
        else if (2 == iType)
        {
            UART_CtrlCmd_Send(CMD_ANDSON_IO, 0); //IO alarm.
        }
    #endif
        ...
        if( iType == 6 )
            sccSetM433Ptz( nReserved );
    }
#if DEV_INFRARED
    extern short int g_LightOpenOnAlarm;
    extern short int g_LightOpenReason;
    extern short int g_LightCloseReason;
    extern unsigned int g_PowerKeyState;
    extern short int g_LightOpenOnAlarmCountTime;
    if(g_LightOpenOnAlarm == 1)
    {
        g_LightOpenOnAlarmCountTime = 0;
        g_LightOpenReason |= ALARMING_OPEN;
    ...
    //if ( ((1 == iType) && (0 == hkSdParam.sdMoveOpen)) || ((2 == iType) && (1 == hkSdParam.sdIoOpen)) )
    {
        if( cur - raise_email > ((ALARMTIME)*3))
        {
            ...
            if(NULL == hk_email.mailserver || NULL == hk_email.passwd || NULL == hk_email.mailfrom || NULL == hk_email.mailto || NULL == hk_email.username)
            {
                printf("email confif is empyt!\n");
                return 0;
            }

            if ((strlen(hk_email.mailserver)==0) || (strlen(hk_email.mailfrom)==0) || ( strlen(hk_email.passwd)==0 ) || (strlen(hk_email.mailto)==0) || (strlen(hk_email.username)==0))
            {
                printf("email confif is empyt!\n");
                //fprintf(stderr, "smtp_server=%s\n send_from=%s\n body=%s\n passwd=%s\n send_to=%s\n", hk_email.mailserver,hk_email.mailfrom,body,passwd,send_to);
                return 0;
            }
...
int MainStreamConfigurate(void)
{
    ...
    int iLight=0;
    sccGetSysInfo( 0, 0, 208, &iLight );
    video_properties_.vv[HKV_iLight] =iLight ;
    int iAlarmLight=0;
    sccGetSysInfo( 0, 0, 204, &iAlarmLight );
    video_properties_.vv[HKV_iAlarmLight]=iAlarmLight ;
    printf("[ %s , %d ].........iLight = %d.... iAlarmLight=%d...\n",__func__,__LINE__,iLight,iAlarmLight);
    return 0;
}
...
#if (DEV_ROBOT)
    if ( (s_FrameRate <= 0) || (s_FrameRate > 10) )
    {
        s_FrameRate = 10; //jisiwei robot.
    }
#else
...
#if (!DEV_ROBOT)
            if(g_AudioSetWifi != g_AUDIO_SET_WIFI_TIME)//exit audio set wifi
            {
...
#define HK_STR_INBAND_DESC2 HK_KEY_FROM"=%s;" \
    HK_KEY_ALERT_INFO"=%s;"\
    HK_KEY_IRATE"=%hhd;"\
    HK_KEY_COMPRESS"=%hhd;" \
    HK_KEY_SENSITIVITY"=%hhd;" \
    HK_KEY_RESOLU"=%d;"
#define HK_STR_INBAND_DIGIT2 HK_KEY_RATE"=%hhd;" \
    HK_KEY_HUE"=%hhd;" \
    ...
    int iThermaiAlarm = 0;
    sccGetSysInfo(0, 0, 110, &iThermaiAlarm);
    int iAudioAlarm = 0;
    sccGetSysInfo(0, 0, 111, &iAudioAlarm);


    len = snprintf(buf, sizeof(buf), HK_STR_INBAND_DESC2 HK_STR_INBAND_DIGIT2,
            getenv("USER"),
            bufMD,
...
static void OnDevNetProxy( HKFrameHead *pFrameHead )
{
    ...
    if ( iSrvID == 0 )
    {
        ...
        if(strncmp( bufValue, "www.uipcam", 10 )==0 )
        {
            ...
            conf_get( CLIENT_CONF_, "SrvIP", cSrvIP, 64);
            if ( strlen(cSrvIP) <= 0 )
            {
                //printf("..730..srvIP=%s..\n",bufValue);
...
void OnMonPtz( const char *ev )
{
    ...
    {
        case 1:		
            UART_CtrlCmd_Send(CMD_LEFT, 0);
            break;
        case 2:
            UART_CtrlCmd_Send(CMD_RIGHT, 0);
            break;
        case 3:
            UART_CtrlCmd_Send(CMD_FORWARD, 0);
            break;
        case 4:
            UART_CtrlCmd_Send(CMD_BACKWARD, 0);
            break;
        default:
            //sleep(1);
            usleep(500*1000);
...
void OnAutoLptspeed(const char* ev )
{
    ...
    */
    ...
    {
    #if (DEV_ROBOT | DEV_ANDSON)
        if( g_jswRunFlag == 0 )
            g_jswRunFlag = 1;
        else
            g_jswRunFlag = 0;
        //UART_CtrlCmd_Send(CMD_STOP, 0); //stop auto clean.
        UART_CtrlCmd_Send(CMD_AUTOCLEAN, 0); //start auto clean.
    #else
        g_PtzRotateEnable = 0;
        g_PtzRotateType = 0;
        g_PtzStepType = 0;	
...
static void OnSetDevAperture( HKFrameHead *pFrameHead)
{
#if (DEV_ROBOT )
    printf("JSW set Power..........\n");
    UART_CtrlCmd_Send(CMD_AUTOCHARGE, 0) ;
#endif 
    return;
}
...
static void OnMonSetAudioAlarm( HKFrameHead *pFrameHead )
{
    int iValue = GetParamUN(pFrameHead,HK_KEY_DEVPARAM );

    sccSetSysInfo( 0,0, 111, iValue );
}

static void OnMonSetThermalAlarm( HKFrameHead *pFrameHead )
{
    int iValue = GetParamUN(pFrameHead,HK_KEY_DEVPARAM );

    sccSetSysInfo( 0,0, 110, iValue );
}
...
//1 open, 2 close
static void OnMonDevLight(HKFrameHead *pFrameHead)
{
    int iLight = GetParamUN( pFrameHead,HK_KEY_DEVPARAM);
    int iFlag = GetParamUN( pFrameHead,HK_KEY_FLAG);//Time open or close

    printf("scc ligth...%d...\n", iLight);
    if( iFlag == 1)
    {
        char *startTime = GetParamStr( pFrameHead, HK_KEY_BEGINE_TIME);//start Open Time
        char *endTime = GetParamStr( pFrameHead, HK_KEY_END_TIME );//Close Time

        printf("scc ligth...%d.startTime=%s..End=%s..\n", iLight,startTime, endTime);
        sccSetOpenOrCloseLight( iLight, startTime, endTime );//hour-min
        return;
    }
    video_properties_.vv[HKV_iLight] = iLight;
    if( iLight==1 )// open
    {
        sccSetSysInfo( 0,0, 208, iLight );
    }
    else if( iLight==2 )// close
    {
        sccSetSysInfo( 0,0, 208, iLight );
    }
}
...
//1 open, 2 close
static void OnMonDevAlarmLight(HKFrameHead *pFrameHead)
{
    int iAlLight = GetParamUN( pFrameHead,HK_KEY_DEVPARAM);
    printf("scc Alarm ligth...%d...\n", iAlLight);

    video_properties_.vv[HKV_iAlarmLight] =iAlLight; 
    if( iAlLight==1 ) //open
    {
        sccSetSysInfo( 0,0, 209, iAlLight );
    }
    else if( iAlLight==2 ) //close
    {
        sccSetSysInfo( 0,0, 209, iAlLight );
    }
}
...
static void OnMonSetDevTranslate( const char * ev )
{
    ...
    switch (mc)
    {
        ...
        case HK_MON_SET_IO_IN://IO IN 0:NO 1:NC
            ...
            break;
        case HK_MON_SET_AUDIOALARM:// 0;  1,2,3
            OnMonSetAudioAlarm( pFrameHead );
            break;
        case HK_MON_SET_THERMALALARM://0, 1
            OnMonSetThermalAlarm( pFrameHead );
            break;
        case HK_MON_SET_ALARM_RNG://RngMD
            OnMonSetAlarmRng( pFrameHead );
            break;
        case HK_MON_DEV_LIGHT: //1 open, 2 close
            OnMonDevLight(pFrameHead);
            break;
        case HK_MON_DEV_ALARM_LIGHT://1 open, 2 close
            OnMonDevAlarmLight(pFrameHead);
            break;
        default:
            break;
    }
...
#endif 
    printf("IO...............\n");
    unsigned int g_AlarmOut = 7;
    unsigned int g_AlarmOutbit = 7; //alarm out:7_7.

    int val_set = 1;
    Hi_SetGpio_SetDir( g_AlarmOut, g_AlarmOutbit, GPIO_WRITE );
    Hi_SetGpio_SetBit( g_AlarmOut, g_AlarmOutbit, val_set ); //pull up.
    usleep(1000*600);
    val_set = 0;
    Hi_SetGpio_SetDir( g_AlarmOut, g_AlarmOutbit, GPIO_WRITE );
    Hi_SetGpio_SetBit( g_AlarmOut, g_AlarmOutbit, val_set ); //pull down.
...
//update dev Net Proxy
static void OnMonDevNetproxy(const char *ev )
{
    HKFrameHead* pFrameHead = CreateFrameA( (char*)ev, strlen(ev) );
    char *cNetIp = GetParamStr( pFrameHead, HK_KEY_PCHOST );
    int iFlag = GetParamUN(pFrameHead,HK_KEY_FLAG);
    if( iFlag == 1 )
    {
    #if WUYUAN_DEBUG
    ...
    if( cNetIp != NULL )
    {
        ...
        conf_set( CLIENT_CONF_, "PROXY", cNetIp );
    }
    DestroyFrame( pFrameHead );
}
...
static int VDoEvent( const char* devname, int obj, const char* ev ) 
{
    ...
    switch(mc)
    {
        ...
        case HK_MON_RESTORATION_PARAM:
            break;
        case HK_MON_DEV_NETPROXY: //update netproxy
            OnMonDevNetproxy( ev );
            break;
        default:
            break;
...
void video_RSLoadObjects(RegisterFunctType reg) 
{
    ...
    if ((0 == g_iCifOrD1) || (g_iCifOrD1 > 5))
    {
        ...
        //if (HK_AR0130 == g_VgaOr720p) 
        //    g_iCifOrD1 = ENUM_960P; //10: 1280*960.
        //else if (HK_OV9712 == g_VgaOr720p)
        //    g_iCifOrD1 = ENUM_720P; //9: 1280*720.
    }
    //else
    //{
    ...
    //if (g_sunCifOrD1 >= 4) //5 ==> VGA:640*480.
    {
        ...
        //if ((HK_AR0130 == g_VgaOr720p) || (HK_OV9712 == g_VgaOr720p))
        //    g_sunCifOrD1 = ENUM_VGA ; //5
        //else
        //    g_sunCifOrD1 = ENUM_PAL_D1; /*704x576*/
    }
    //else
    //{
    //    VeSunChn = 1;  //Venc Channel 1: VGA.
    //    if ((HK_AR0130 == g_VgaOr720p) || (HK_OV9712 == g_VgaOr720p))
    //        g_sunCifOrD1 = ENUM_VGA;//3
    //    else
    //        g_sunCifOrD1 = ENUM_CIF;       /*352x288*/
    //}
