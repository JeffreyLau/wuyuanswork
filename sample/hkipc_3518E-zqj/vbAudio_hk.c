...
#if (AUDIO_DETECT)
/*************   Voice  alarm   ***************/
extern int get_chkaudioalarm_buf_flag;
extern char chkAudioBuf[320*5];
...
HI_S32 SAMPLE_Audio_AiAenc( int iSamplerate)
{
    HI_S32 s32ret = 0, i = 0;
    ...
    stAioAttr.enSamplerate   = iSamplerate;//AUDIO_SAMPLE_RATE_8000;
    ...
    ********************************************/
    ...
    SAMPLE_COMM_AUDIO_StopAi(s_AiDev, s32AiChnCnt, gs_bAiAnr, HI_TRUE);//sun
    usleep(3000);
...
HI_S32 LPCM_Audio_AdecAo()
{
    ...
    s32ret = SAMPLE_COMM_AUDIO_StartAdec(s_AdecChn, PT_LPCM);
    if (s32ret != HI_SUCCESS)
    {
        SAMPLE_DBG(s32ret);
        return HI_FAILURE;
...
HI_S32 SAMPLE_Audio_AdecAo()
{
     ...
     *  step 2: start Adec
     *********************************************/
    SAMPLE_COMM_AUDIO_StopAo(s_AoDev, s_AoChn, HI_FALSE);
    usleep(2000);
    SAMPLE_COMM_AUDIO_StopAdec(s_AdecChn);
    usleep(3000);
    //s32ret = SAMPLE_COMM_AUDIO_StartAdec(g_AdecChn, gs_enPayloadType);
...
static int scc_AudioOpen(const char *name, const char *args, int *threadreq) 
{
    ...
    {
        if(HI_SUCCESS != SAMPLE_Audio_AiAenc(AUDIO_SAMPLE_RATE_8000))
        {
            return -1;
        }
        *threadreq = 1;
...
//audio get wifi-ssid,pwd
static void AudioGetWifiConf( )
{
	printf("scc AudioGetWifiConf .......\n");
    if(HI_SUCCESS != SAMPLE_Audio_AiAenc(AUDIO_SAMPLE_RATE_44100))
    {
        printf("scc AudioGetWifiConf Failed.......\n");
        return;
    }
	...
	{	
		...
		loop = 4;
		while (loop-- > 0 && g_AudioSetWifi!= g_AUDIO_SET_WIFI_TIME )
		{     
			TimeoutVal.tv_sec = 1;
			...
			if (FD_ISSET(g_AencFd, &read_fds))
			{
                ...
                write_audioData( stStream.pStream, stStream.u32Len );
...
static void AudioThread(void)
{
    ...
    #if (AUDIO_DETECT)
        //printf("audio_alarm lv  = %d  ,  bytes = %d\n",audio_alarm,bytes);
        if ( (get_chkaudioalarm_buf_flag == 0) && (audio_alarm > 1) )
        {
            memset(chkAudioBuf, 0, 320*5);
            memcpy(chkAudioBuf, buf, bytes);
            get_chkaudioalarm_buf_flag = 1;
        }
    #endif
