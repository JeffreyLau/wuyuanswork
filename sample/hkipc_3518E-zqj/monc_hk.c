...
extern Dict *g_TFPacket;
extern int g_iTfCount;
    ...
    if( g_iTfCount > 0)
    {
        ...
        for( j=0; j<pageSize; j++)
        {
            ...
            if(cFile != NULL)
            {
                ...
                char cNewFile[64]={0};
                strcpy( cNewFile, cFile+9 );// 20150802/
        ...
        DictSetInt( DictPacket, HK_KEY_LISTCOUNT, g_iTfCount-g_deleteTfData );
    }
    else
    {
...
static void OnWanGetDevParam(int nCmd, Dict *hf, int mc )
{
    ...
    else
    {
	  ...
	  char cStartTime[128]={0};
    char cEndTime[128]={0};
    int iFlag=0;
    sccGetOpenOrCloseLight( &iFlag, &cStartTime, &cEndTime);
    DictSetInt( DictPacket, HK_KEY_LIGHT,video_properties_.vv[HKV_iLight]);
    DictSetInt( DictPacket, HK_KEY_ALARMLIGHT,video_properties_.vv[HKV_iAlarmLight]);
    DictSetInt( DictPacket, HK_KEY_LIGHTTIME, iFlag );
    DictSetStr(DictPacket, HK_KEY_BEGINE_TIME, cStartTime );
    DictSetStr(DictPacket, HK_KEY_END_TIME, cEndTime );
    int iAudioAlarm = 0;
    sccGetSysInfo( 0, 0, 111, &iAudioAlarm );
    //1-close 2 3 4
    DictSetInt( DictPacket, HK_KEY_AUDIOALARM, iAudioAlarm);
...
void OnRestorationParam( )
{
    ...
    conf_set_int(HOME_DIR"/hkipc.conf", "m433_state", 1);
    ...
    conf_set_int(HOME_DIR"/web.conf", "bOnvifAudio", 0); //disable onvif audio.  

    /**ptz.conf**/
    conf_set_int(HOME_DIR"/ptz.conf", "preset1x", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset1y", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset2x", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset2y", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset3x", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset3y", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset4x", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset4y", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset5x", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset5y", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset6x", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset6y", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset7x", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset7y", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset8x", 0);
    conf_set_int(HOME_DIR"/ptz.conf", "preset8y", 0);
...
static void OnAlarmParam( Dict *d, const char *buf )
{
    ...
    sccSetSysInfo( 0,0, 111, iAudioAlarm );
...
static void OnWanControlDev( int nCmd, int iSubCmd, Dict *d )
{
    char *cDevid = DictGetStr( d, HK_KEY_FROM );
    char *cUserid = DictGetStr( d, HK_KEY_TO );
    unsigned int ulParam = DictGetInt( d, HK_KEY_UIPARAM );
    ...
    printf("scc 11111111ControlDev Cmd=%d,,,values=%d...\n", iCmd, iValues);
    if( iCmd == 210 )
    {
        char *cCode = DictGetStr(d, HK_KEY_CHANNEL);
        printf("scc...iCode=%d...cCode=%s..\n", iValues, cCode );
        Dict *DictPacket = DictCreate( 0, 0 );
        DictSetInt( DictPacket, HK_KEY_MAINCMD, HK_MON_GET_DEVPARAM );
        DictSetInt( DictPacket, HK_KEY_SUBCMD, HK_MON_GET_CONTROL_DEV );

        DictSetInt( DictPacket, HK_KEY_DEVPARAM, iCmd );
        int i=0;
        for(i=0; i< 32; i++)
        {
            char keyBuf2[12]={0};
            sprintf(keyBuf2, "code%d", i );
            m433_delete_code(i , keyBuf2, iValues);

            char keyBuf[12]={0};
            sprintf(keyBuf, "%d", i );
            char cTemCode[1024]={0};
            conf_get( HOME_DIR"/m433Code.conf", keyBuf, cTemCode, 1024);
            if(strlen(cTemCode) > 2 && iValues > 0 )
            {
                char *pCode = strstr(cTemCode, cCode);
                if( pCode != NULL)
                {
                    DictSetInt( DictPacket, HK_KEY_FLAG, 1 );
                    conf_set(HOME_DIR"/m433Code.conf", keyBuf, "0");
                }
            }
        }
        ...
        NetSend( HK_KEY_MONSERVER, cBuf, iPacketLen );
        printf("scc.208.433.OnWebGetNetInfo..cBuf=%s...\n", cBuf );
        DictDestroy( DictPacket );
        return;
    }
    else if( iCmd == 208 )
    ...
    else if( iCmd == 209 )
    {
        ...
        DictSetInt( DictPacket, HK_KEY_FLAG, 1 );
        int nValue = 0;
        sccGetSysInfo( NULL, 0, iCmd, &nValue );
        ...
        for(i=0; i< 32; i++)
        {
            ...
            {
                sprintf(keyBuf, "%d", iCount++ );
                DictSetStr( DictPacket, keyBuf, cTemCode );
                printf("se m433 count=%d buf=%s.\n", i, cTemCode);
            }
        }
        ...
        NetSend( HK_KEY_MONSERVER, cBuf, iPacketLen );
        printf("scc.209.433..OnWebGetNetInfo..cBuf=%s...\n", cBuf );
        DictDestroy( DictPacket );
        return;
    }

    printf("scc ControlDev Cmd=%d,,,values=%d...\n", iCmd, iValues);
    sccSetSysInfo(NULL, 0, iCmd, iValues);
}
...
static void OnGetOnvifInfo(int nCmd, int hkid ,int iSubCmd, Dict *d)
{
    ...
    int isOpenAudio = conf_get_int(HOME_DIR"/web.conf", "bOnvifAudio" );
    printf(".....%s....isOpen:%d....\n", __func__, isOpen);
    ...
    DictSetInt( DictPacket, HK_KEY_OPENONVIFAUDIO, isOpenAudio );
    DictSetInt( DictPacket, HK_KEY_UIPARAM, ulParam );
    DictSetStr( DictPacket, HK_KEY_TO, getenv("USER"));
    DictSetInt( DictPacket, HK_KEY_FLAG, 1 );
...
{
    //printf("[%s, %d] send test email, b_testEmail: %d....\n", __func__, __LINE__, (b_testEmail == true)?1:0);
    if (! b_testEmail)
        return;
...
static void OnSetOnvifInfo(int nCmd,int iSubCmd, int hkid, Dict *d)
{
    ...
    int isOpenAudio = DictGetInt( d, HK_KEY_OPENONVIFAUDIO);
    ...
    conf_set_int(HOME_DIR"/web.conf", "bOnvifAudio", isOpenAudio );

    sys_restart( );
}

//code=;home=;homeflag=;point=;
static void OnSetMon433(int nCmd, int iSubCmd, int hkid, Dict *d)
{
    ...
    int iPoint = DictGetInt( d, "point");

    Dict *DictPacket = DictCreate( 0, 0 );
    if( cCode != NULL )
    {
        char cCodeBuf[1024]={0};
        sprintf(cCodeBuf, "%s-%d-%d-%d", cCode, iHome, iHomeFlag, iPoint);
        ...
        for(i=0; i< 32; i++)
        {
            ...
            if( pCode != NULL)
            {
                conf_set(HOME_DIR"/m433Code.conf", keyBuf, cCodeBuf);
                break;
            }
            else if(strlen(cTemCode) <= 2 )
            {
                conf_set(HOME_DIR"/m433Code.conf", keyBuf, cCodeBuf);
                printf("se m433 count=%d buf=%s.\n", i, cCodeBuf);
                break;
        ...
        DictSetInt( DictPacket, HK_KEY_FLAG, 0 );
